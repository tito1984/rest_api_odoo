...
class RestApi(http.Controller):
        ...
        try:
            if method == 'GET':
                ...
                if not option.is_get:
                    ...
                    resp = json.loads(values)
                    domain = [(key, '=', value) for key, value in resp.items()]
        ...
        if method == 'POST':
            if not option.is_post:
                ...
                try:
                    ...
                    new_resource = request.env[str(model_name)].write(
                        data['values'])
    ...
    def fetch_data(self, **kw):
        ...
        if not model_id:
            return request.make_response(data=json.dumps({
                "Status": "failed",
                "Error": "Invalid model, check spelling or maybe "
                    "the related "
